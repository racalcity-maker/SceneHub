...
static bool optional_identifier_valid(const char *text, size_t cap)
{
    return !text || text[0] == '\0' || node_text_identifier_valid(text, cap - 1U);
}
...
static bool known_card_valid(const node_nfc_known_card_t *card)
{
    ...
    if (!optional_identifier_valid(card->event_name, sizeof(card->event_name))) {
        return false;
    }
    return true;
}
...
esp_err_t node_driver_nfc_contract_validate_config(const node_nfc_reader_config_t *config,
                                                   node_driver_instance_info_t *out_instance)
{
    ...
    if (!node_text_identifier_valid(config->id, sizeof(config->id) - 1U) ||
        !node_text_identifier_valid(config->driver_impl, sizeof(config->driver_impl) - 1U) ||
        !node_text_identifier_valid(config->bus, sizeof(config->bus) - 1U)) {
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t node_driver_nfc_contract_resolve_card(const node_nfc_reader_config_t *config,
                                                const char *uid,
