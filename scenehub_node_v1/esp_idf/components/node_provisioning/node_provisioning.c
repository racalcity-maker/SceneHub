...
static void make_ap_ssid(const node_config_t *config, char *out, size_t out_size);
...
static void make_ap_ssid(const node_config_t *config, char *out, size_t out_size)
{
    const char *id = (config && config->node_id[0]) ? config->node_id : "node";
    int written = snprintf(out, out_size, "SHN-");
    if (written < 0 || written >= (int)out_size) {
        return;
    }

    size_t pos = (size_t)written;
    for (size_t i = 0; id[i] && pos + 1 < out_size && pos < 32; ++i) {
        char ch = id[i] == '_' ? '-' : id[i];
        bool ok = (ch >= 'a' && ch <= 'z') ||
                  (ch >= 'A' && ch <= 'Z') ||
                  (ch >= '0' && ch <= '9') ||
...
{
    char ssid[33];
    make_ap_ssid(config, ssid, sizeof(ssid));
    ...
    ESP_LOGW(TAG, "provisioning AP started ssid=%s", ssid);
