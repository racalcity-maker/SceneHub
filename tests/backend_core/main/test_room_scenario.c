...
EXT_RAM_BSS_ATTR static quest_device_t s_room_scenario_device;
...
static bool room_scenario_test_has_device_ref(
    char refs[][ROOM_SCENARIO_DEVICE_ID_MAX_LEN],
    size_t count,
    const char *device_id)
{
    for (size_t i = 0; i < count; ++i) {
        if (strcmp(refs[i], device_id) == 0) {
            return true;
        }
    }
    return false;
...
static void upsert_compact_node_for_validation(void)
{
    const char *json =
        "{\"id\":\"node_1\",\"client_id\":\"dcc-node-1\",\"name\":\"Node 1\",\"enabled\":true,"
        "\"device_description\":{"
        "\"manifest_version\":2,\"format\":\"compact_resources\","
        "\"node_kind\":\"scenehub_node\","
        "\"capability_contract\":\"scenehub.node.compact.v1\","
        "\"device\":{\"id\":\"node_1\",\"name\":\"Node 1\",\"kind\":\"scenehub_node\"},"
        "\"resources\":{\"relays\":[{\"channel\":1,\"label\":\"Relay 1\"}],"
        "\"mosfets\":[{\"channel\":1,\"label\":\"MOSFET 1\"}],"
        "\"inputs\":[],\"outputs\":[],\"led_strips\":[]},"
        "\"command_templates\":[{\"id\":\"relay.set\",\"label\":\"Relay set\","
        "\"target\":\"relays\",\"command\":\"relay.set\",\"args_schema_ref\":\"output_set\","
        "\"policy\":{\"manual_allowed\":true,\"scenario_allowed\":true,"
        "\"requires_confirmation\":false,\"result_required\":false,"
        "\"timeout_ms\":3000,\"danger_level\":\"normal\"}},"
        "{\"id\":\"mosfet.effect\",\"label\":\"MOSFET effect\","
        "\"target\":\"mosfets\",\"command\":\"mosfet.effect\",\"args_schema_ref\":\"effect\","
        "\"policy\":{\"manual_allowed\":true,\"scenario_allowed\":true,"
        "\"requires_confirmation\":false,\"result_required\":false,"
        "\"timeout_ms\":3000,\"danger_level\":\"normal\"}}],"
        "\"event_templates\":[],"
        ...
        "\"options\":[\"blink\",\"fade_in\"]}]}}}";
    cJSON *root = cJSON_Parse(json);
    TEST_ASSERT_NOT_NULL(root);
    memset(&s_room_scenario_device, 0, sizeof(s_room_scenario_device));
    TEST_ASSERT_EQUAL(ESP_OK, quest_device_from_json(root, &s_room_scenario_device));
    cJSON_Delete(root);
    TEST_ASSERT_EQUAL(ESP_OK, quest_device_upsert(&s_room_scenario_device));
}

static void test_room_scenario_runtime_validation_resolves_compact_node_commands(void)
{
    room_scenario_t *scenario = &s_room_scenario_work[0];
    room_scenario_validation_report_t report = {0};
    ...
    TEST_ASSERT_EQUAL(ESP_OK, quest_device_init());
    TEST_ASSERT_EQUAL(ESP_OK, quest_device_clear());
    upsert_compact_node_for_validation();
    memset(s_room_scenario_work, 0, sizeof(s_room_scenario_work));

    init_scenario(scenario, "compact_valid", "room_a", "Compact valid");
    fill_device_command_step(&scenario->steps[0], "run", "Run", "node_1", "relay.set");
    set_text(scenario->steps[0].data.device_command.params_json,
             sizeof(scenario->steps[0].data.device_command.params_json),
             "{\"channel\":1,\"on\":true}");
    scenario->step_count = 1;

    TEST_ASSERT_EQUAL(ESP_OK, scenehub_scenario_validate_environment(scenario, &report));
    TEST_ASSERT_TRUE(report.valid);
    TEST_ASSERT_EQUAL_UINT32(0, report.issue_count);
    ...
    TEST_ASSERT_FALSE(report.valid);
    TEST_ASSERT_TRUE(report.issue_count >= 1);
    TEST_ASSERT_EQUAL_STRING("DEVICE_COMMAND_INVALID", report.issues[0].code);
    ...
    memset(&report, 0, sizeof(report));
    set_text(scenario->steps[0].data.device_command.command_id,
             sizeof(scenario->steps[0].data.device_command.command_id),
             "mosfet.effect");
    set_text(scenario->steps[0].data.device_command.params_json,
             sizeof(scenario->steps[0].data.device_command.params_json),
             "{\"channel\":1}");
    ...
    TEST_ASSERT_FALSE(report.valid);
    TEST_ASSERT_TRUE(report.issue_count >= 1);
    TEST_ASSERT_EQUAL_STRING("DEVICE_COMMAND_NOT_FOUND", report.issues[0].code);
}
...
static void test_room_scenario_collect_device_refs_includes_reactive_graph(void)
{
    room_scenario_t scenario = {0};
    char refs[8][ROOM_SCENARIO_DEVICE_ID_MAX_LEN] = {0};
    size_t count = 0;

    init_scenario(&scenario, "scenario_refs", "room_a", "Scenario Refs");
    scenario.reactive_action_count = 1;
    scenario.reactive_actions[0].type = ROOM_SCENARIO_STEP_DEVICE_COMMAND;
    set_text(scenario.reactive_actions[0].data.device_command.device_id,
             sizeof(scenario.reactive_actions[0].data.device_command.device_id),
             "relay_action");
    set_text(scenario.reactive_actions[0].data.device_command.command_id,
             sizeof(scenario.reactive_actions[0].data.device_command.command_id),
             "pulse");
    scenario.reactive_actions[0].group_command_start_index = 0;
    scenario.reactive_actions[0].group_command_count = 1;
    scenario.reactive_group_command_count = 1;
    set_text(scenario.reactive_group_commands[0].device_id,
             sizeof(scenario.reactive_group_commands[0].device_id),
             "relay_group");
    set_text(scenario.reactive_group_commands[0].command_id,
             sizeof(scenario.reactive_group_commands[0].command_id),
             "set");
    scenario.branch_count = 1;
    scenario.branches[0].type = ROOM_SCENARIO_BRANCH_REACTIVE;
    scenario.branches[0].enabled = true;
    scenario.branches[0].trigger.kind = ROOM_SCENARIO_REACTIVE_TRIGGER_DEVICE_EVENT;
    set_text(scenario.branches[0].trigger.device_id,
             sizeof(scenario.branches[0].trigger.device_id),
             "uid_gate");
    set_text(scenario.branches[0].trigger.event_id,
             sizeof(scenario.branches[0].trigger.event_id),
             "card");

    TEST_ASSERT_EQUAL(ESP_OK,
                      room_scenario_collect_device_refs(&scenario,
                                                        refs,
                                                        sizeof(refs) / sizeof(refs[0]),
                                                        &count));
    TEST_ASSERT_TRUE(room_scenario_test_has_device_ref(refs, count, "uid_gate"));
    TEST_ASSERT_TRUE(room_scenario_test_has_device_ref(refs, count, "relay_action"));
    TEST_ASSERT_TRUE(room_scenario_test_has_device_ref(refs, count, "relay_group"));
}
...
void register_room_scenario_tests(void)
{
    ...
    RUN_TEST(test_room_scenario_runtime_validation_resolves_compact_node_commands);
    ...
    RUN_TEST(test_room_scenario_collect_device_refs_includes_reactive_graph);
