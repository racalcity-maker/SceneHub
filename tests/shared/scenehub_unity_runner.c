...
{
    const scenehub_test_suite_t *suite = (const scenehub_test_suite_t *)arg;
    ...
    UNITY_BEGIN();
    for (size_t i = 0; suite && i < suite->registrar_count; ++i) {
        if (suite->registrars[i]) {
            suite->registrars[i]();
        }
    }
    UNITY_END();
...
void scenehub_test_runner_start(const scenehub_test_suite_t *suite)
{
    ...
    BaseType_t ok = xTaskCreateWithCaps(unity_runner_task,
                                        "unity_runner",
