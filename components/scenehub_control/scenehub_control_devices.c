                                              ...
                                              scenehub_control_result_t *out_result)
{
    ...
    err = scenehub_control_dispatch_device_command(source,
                                                   device_id,
                                                   command_id,
                                                   params_json,
                                                   out_info,
    ...
    if (dispatch.request_id[0]) {
        scenehub_control_set_request_id(out_result, dispatch.request_id);
        scenehub_control_log_device_action(source,
                                           device_id,
                                           log_warning,
                                           command_id,
                                           dispatch.request_id);
        scenehub_control_finish_accepted_with_invalidation(out_result,
    ...
    scenehub_control_result_t *out_result)
{
    ...
    err = scenehub_control_dispatch_describe_interface(client_id, out_info, out_result);
    if (err != ESP_OK) {
        scenehub_control_fill_common_error(out_result, err);
    }
    return ESP_OK;
