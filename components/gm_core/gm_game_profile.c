...
static void *gm_game_profile_heap_alloc(size_t size)
{
...
esp_err_t gm_game_profile_init(void)
{
    ...
    return gm_game_profile_ensure_persist_lock();
}
...
esp_err_t gm_game_profile_validate(const gm_game_profile_t *profile)
{
    ...
    scenario = gm_game_profile_heap_alloc(sizeof(*scenario));
    if (!scenario) {
        return ESP_ERR_NO_MEM;
    }
    ...
    if (strcmp(scenario->room_id, profile->room_id) != 0) {
        heap_caps_free(scenario);
        return ESP_ERR_INVALID_STATE;
    }
    report = gm_game_profile_heap_alloc(sizeof(*report));
    if (!report) {
        heap_caps_free(scenario);
        return ESP_ERR_NO_MEM;
    }
