    ...
    gm_room_session_command_plan_t *out_plan)
{
    ...
    case ROOM_SCENARIO_REACTIVE_RESULT_CONTINUE:
        return reactive_v2_finish_locked(session, branch, model, now_ms, out_plan);
                                                      ...
                                                      gm_room_session_command_plan_t *out_plan)
{
    ...
    if (!session || !branch || !model || !branch->active || !out_plan) {
        return ESP_ERR_INVALID_ARG;
    }
    ...
    while (budget-- > 0) {
        ...
        case ROOM_SCENARIO_STEP_DEVICE_COMMAND: {
            esp_err_t err = gm_room_session_plan_reactive_action_command_locked(session,
                                                                                branch,
                                                                                action,
                                                                                now_ms,
                                                                                command_error,
