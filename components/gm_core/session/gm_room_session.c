...
} gm_room_session_deferred_side_effects_t;

static gm_room_session_deferred_side_effects_t s_deferred_side_effects = {0};
...
void gm_room_session_runtime_wake(void)
{
    ...
    (void)xQueueSend(s_runtime_queue, &cause, 0);
}
...
static esp_err_t gm_room_session_ensure_event_worker(void)
{
    ...
    if (!s_event_queue) {
        ...
        if (!s_event_queue) {
            s_event_queue = xQueueCreate(GM_ROOM_SESSION_EVENT_QUEUE_LEN, sizeof(scenehub_event_t));
        }
        if (!s_event_queue) {
...
static esp_err_t gm_room_session_ensure_runtime_worker(void)
{
    ...
    if (!s_runtime_queue) {
        s_runtime_queue = xQueueCreateWithCaps(GM_ROOM_SESSION_RUNTIME_QUEUE_LEN,
                                               sizeof(gm_room_runtime_cause_t),
                                               MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (!s_runtime_queue) {
            s_runtime_queue = xQueueCreate(GM_ROOM_SESSION_RUNTIME_QUEUE_LEN,
                                           sizeof(gm_room_runtime_cause_t));
        }
        if (!s_runtime_queue) {
            return ESP_ERR_NO_MEM;
...
esp_err_t gm_room_session_sessions_lock(void)
{
    ...
    s_sessions_lock_epoch++;
    if (s_sessions_lock_epoch == 0) {
        s_sessions_lock_epoch = 1;
    }
    return ESP_OK;
}
...
void gm_room_session_sessions_unlock(void)
{
    ...
    if (s_sessions_mutex) {
        cancel_count = s_deferred_side_effects.cancel_count;
        flag_event_count = s_deferred_side_effects.flag_event_count;
        if (cancel_count > 0) {
            memcpy(cancel_request_ids,
                   s_deferred_side_effects.cancel_request_ids,
                   sizeof(cancel_request_ids));
        }
        if (flag_event_count > 0) {
            memcpy(flag_events, s_deferred_side_effects.flag_events, sizeof(flag_events));
        }
        memset(&s_deferred_side_effects, 0, sizeof(s_deferred_side_effects));
    ...
    for (size_t i = 0; i < flag_event_count; ++i) {
        ...
        if (!flag_events[i].name[0]) {
            continue;
        }
        if (scenehub_event_make_flag_changed(&message,
                                             flag_events[i].name,
                                             flag_events[i].value) != ESP_OK) {
            ESP_LOGW(TAG, "failed to build deferred flag event: room flag=%s", flag_events[i].name);
            continue;
        }
...
void gm_room_session_mark_session_changed_locked(gm_room_session_t *session)
{
    ...
    if (session->last_change_lock_epoch != s_sessions_lock_epoch) {
        session->generation = ++s_generation;
        session->last_change_lock_epoch = s_sessions_lock_epoch;
    }
    quest_str_copy(s_last_changed_room_id,
                   sizeof(s_last_changed_room_id),
                   session->room_id);
}
...
void gm_room_session_defer_cancel_request_locked(const char *request_id)
{
    ...
    for (size_t i = 0; i < s_deferred_side_effects.cancel_count; ++i) {
        if (strcmp(s_deferred_side_effects.cancel_request_ids[i], request_id) == 0) {
            return;
        }
    }
    if (s_deferred_side_effects.cancel_count >= GM_ROOM_SESSION_DEFERRED_CANCEL_MAX) {
        ESP_LOGW(TAG, "deferred cancel queue full, dropping request_id=%s", request_id);
        return;
    }
    quest_str_copy(s_deferred_side_effects
                       .cancel_request_ids[s_deferred_side_effects.cancel_count],
                   sizeof(s_deferred_side_effects.cancel_request_ids[0]),
                   request_id);
    s_deferred_side_effects.cancel_count++;
}
...
void gm_room_session_scenario_clear_wait_locked(gm_room_session_t *session)
{
    ...
    if (session->wait_type == GM_ROOM_SCENARIO_WAIT_DEVICE_COMMAND_RESULT &&
        ...
        gm_room_session_defer_cancel_request_locked(session->wait_event_type);
    }
...
post_changed:
    for (size_t i = 0; i < s_deferred_side_effects.flag_event_count; ++i) {
        if (strcmp(s_deferred_side_effects.flag_events[i].name, name) == 0) {
            s_deferred_side_effects.flag_events[i].value = value;
            return ESP_OK;
        }
    }
    if (s_deferred_side_effects.flag_event_count >= GM_ROOM_SCENARIO_MAX_FLAGS) {
        ESP_LOGW(TAG, "deferred flag event queue full, dropping flag=%s", name);
        return ESP_ERR_NO_MEM;
    }
    quest_str_copy(s_deferred_side_effects.flag_events[s_deferred_side_effects.flag_event_count].name,
                   sizeof(s_deferred_side_effects.flag_events[0].name),
                   name);
    s_deferred_side_effects.flag_events[s_deferred_side_effects.flag_event_count].value = value;
    s_deferred_side_effects.flag_event_count++;
    return ESP_OK;
}
...
void gm_room_session_reset_all(void)
{
    ...
    if (s_runtime_queue) {
        (void)xQueueReset(s_runtime_queue);
    }
...
{
    if (!out_room_id || out_room_id_size == 0) {
        return ESP_ERR_INVALID_ARG;
    }
    out_room_id[0] = '\0';
    if (gm_room_session_sessions_lock() != ESP_OK) {
    ...
    quest_str_copy(out_room_id, out_room_id_size, s_last_changed_room_id);
    gm_room_session_sessions_unlock();
    return out_room_id[0] ? ESP_OK : ESP_ERR_NOT_FOUND;
}
