...
static bool ap_stop_pending_value(void)
{
    bool pending = false;
    state_lock();
    pending = s_ap_stop_pending;
    state_unlock();
    return pending;
}

static void ap_stop_pending_set(bool pending)
{
    state_lock();
    s_ap_stop_pending = pending;
    state_unlock();
}
...
static void on_ip_event(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP) {
        ...
        if (ap_mode_value()) {
            ...
            if (err != ESP_OK) {
                ESP_LOGW(TAG, "failed to schedule AP stop after STA IP: %s", esp_err_to_name(err));
            }
        }
    }
}
...
static void stop_ap_task(void *param)
{
    ...
    /*
     * Give the HTTP response that requested AP shutdown time to leave the AP
     * socket before changing Wi-Fi mode. Switching mode inside the handler can
     * tear down the interface that owns the active request.
     */
    vTaskDelay(pdMS_TO_TICKS(2000));

    if (ap_mode_value()) {
        ESP_LOGI(TAG, "stopping AP, deauth clients");
        esp_err_t err = esp_wifi_deauth_sta(0);
        if (err != ESP_OK && err != ESP_ERR_WIFI_NOT_STARTED) {
            ESP_LOGW(TAG, "esp_wifi_deauth_sta failed: %s", esp_err_to_name(err));
        }
...
esp_err_t network_stop_ap(void)
{
    ...
    if (ap_stop_pending_value()) {
        return ESP_OK;
    }

    ap_stop_pending_set(true);
    BaseType_t ok = xTaskCreate(stop_ap_task,
                                "wifi_ap_stop",
                                4096,
                                NULL,
                                3,
                                &s_ap_stop_task);
    if (ok != pdPASS) {
        ap_stop_pending_set(false);
        s_ap_stop_task = NULL;
        ESP_LOGE(TAG, "failed to create AP stop task");
        return ESP_ERR_NO_MEM;
    }
    ESP_LOGI(TAG, "AP stop scheduled");
    return ESP_OK;
}
