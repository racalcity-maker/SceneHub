...
"function applyRoleLayout(role){const body=document.body;if(body){body.dataset.role=role||'admin';}if(document.body){document.body.classList.remove('role-user');}tabs.forEach(tab=>{tab.style.display='';});}"
...
"function finishScrub(){progressScrubbing=false;updateProgressDisplay();commitScrub();}"
        ...
        "refreshModeButtons();"
"sessionRolePromise.then(info=>{if(info&&info.role==='user'){return;}loadStatus().then(()=>{if(statusInterval)clearInterval(statusInterval);statusInterval=setInterval(()=>{const activePane=document.querySelector('.pane.active');if(!activePane)return;const activeId=activePane.id;if(activeId==='pane-audio'){fetchAudioStatus();return;}if(activeId==='pane-status'){loadStatus().catch(()=>{});}},3000);});});"
"</script>"
        "</div>"
        "</body></html>";
