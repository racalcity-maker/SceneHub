...
esp_err_t status_handler(httpd_req_t *req)
{
    ...
    if (!wifi || !mqtt || !audio || !web || !web_operator || !sd || !diag || !mem ||
        !dram || !psram || !clients || !ota_obj || !services || !mqtt_users) {
        if (mqtt_users) {
    ...
    cJSON_AddNumberToObject(svc_event_bus, "event_queue_waiting", event_bus_status.event_queue_waiting);
    cJSON_AddNumberToObject(svc_event_bus, "event_slow_handlers", event_bus_status.event_slow_handlers);
    cJSON_AddNumberToObject(svc_event_bus, "event_max_handler_ms", event_bus_status.event_max_handler_ms);
    cJSON_AddNumberToObject(svc_event_bus, "job_posted", event_bus_status.event_job_posted);
